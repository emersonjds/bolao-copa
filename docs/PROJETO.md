...
| Banco (Vitest+`pg`, node)           | `pnpm test:db`  | regra de pontos, multiplicador, pênalti, idempotência, trava, **desempate**, grants, **round-trip de backup** | 32 testes                      |
...
- ⏳ B-1 (convite uso único) e M-3 (filtro por `bolao_id`): adiados — só relevantes quando houver fluxo de convite / múltiplos bolões. Habilitar **PITR** em prod (ação no painel) — mitigado pelo backup diário em JSON (repo privado `backup-bolao-da-copa`).
