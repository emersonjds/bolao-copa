...
| M-1 | 🟡 Médio | `is_admin` legível por todos os autenticados (enumeração de admins) | ⏳ Pendente (column-level select; exige função SECURITY DEFINER pra `use-is-admin`) |
...
| B-1 | 🔵 Baixo | Convite sem uso único (link reutilizável) | ⏳ Pendente |
...
2. Resolver M-1 (esconder `is_admin` de leitura geral) e B-1 (convite uso único) antes de abrir o bolão além da validação privada.
