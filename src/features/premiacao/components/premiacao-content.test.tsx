...
describe("PremiacaoContent", () => {
  it("mostra a regra de divisão 50/30/20 e a inscrição", () => {
    mock.mockReturnValue({ data: undefined, isLoading: false, isError: false } as ReturnType<typeof useContagemInscritos>);
  ...
  it("com contagem, mostra o pote e os valores por colocação", () => {
    mock.mockReturnValue({ data: 87, isLoading: false, isError: false } as ReturnType<typeof useContagemInscritos>);
    render(<PremiacaoContent />);
  ...
  it("sem contagem, degrada para a regra em % sem quebrar", () => {
    mock.mockReturnValue({ data: undefined, isLoading: false, isError: true } as ReturnType<typeof useContagemInscritos>);
    render(<PremiacaoContent />);
    expect(screen.getByText(/50%/)).toBeInTheDocument();
