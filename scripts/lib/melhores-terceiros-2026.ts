// Tabela oficial FIFA World Cup 2026 — alocação dos 8 melhores 3os colocados (Anexo C
// do regulamento do torneio). Gerado a partir das 495 combinacoes publicadas; cada chave
// e a combinacao ordenada dos 8 grupos cujo 3o colocado se classificou.
// Fonte: en.wikipedia.org/wiki/2026_FIFA_World_Cup_knockout_stage (Template:2026 FIFA World
// Cup third-place table) -> FIFA Tournament Regulations, Annex C.

export type Grupo = "A" | "B" | "C" | "D" | "E" | "F" | "G" | "H" | "I" | "J" | "K" | "L";

// O slot e identificado pelo cabeca de chave (vencedor do grupo) que enfrenta o 3o colocado.
export type SlotTerceiro = "1E" | "1I" | "1A" | "1L" | "1D" | "1G" | "1B" | "1K";

// Conjunto de grupos cujo 3o colocado pode cair em cada slot (Anexo C / seed.sql trinta-e-dois).
export const CANDIDATOS_SLOT: Record<SlotTerceiro, Grupo[]> = {
  "1E": ["A", "B", "C", "D", "F"],
  "1I": ["C", "D", "F", "G", "H"],
  "1A": ["C", "E", "F", "H", "I"],
  "1L": ["E", "H", "I", "J", "K"],
...
const TABELA_OFICIAL: Record<string, Record<SlotTerceiro, Grupo>> = {
...
/**
 * Recebe exatamente os 8 grupos cujos 3os colocados se classificaram e devolve, conforme a
 * tabela oficial do Anexo C, qual grupo ocupa cada slot (1A, 1B, 1D, 1E, 1G, 1I, 1K, 1L).
 * Lanca se nao receber 8 grupos distintos ou se a combinacao nao existir na tabela.
 */
export function atribuirTerceiros(gruposQualificados: Grupo[]): Record<SlotTerceiro, Grupo> {
  const distintos = new Set(gruposQualificados);
  if (gruposQualificados.length !== 8 || distintos.size !== 8) {
    throw new Error(
      `atribuirTerceiros exige 8 grupos distintos; recebeu ${gruposQualificados.length} (${distintos.size} distintos).`,
    );
  }
  const chave = [...gruposQualificados].sort().join("");
  const linha = TABELA_OFICIAL[chave];
  if (!linha) throw new Error(`Combinacao de 3os colocados desconhecida: ${chave}.`);
  return linha;
}
